...
// 1. SKILL ICON MAPPING (Same as before)
const SKILL_ICONS = {
  ...
  // "OpenWeather API": { icon: SiOpenapi, color: "#6BA539" },
  // API: { icon: SiOpenapi, color: "#6BA539" },
};
...
const ProjectCard = ({ project, index }) => {
  return (
    <div className="group flex flex-col gap-5 w-full">
      {/* IMAGE CONTAINER FIX:
         - Changed bg-zinc-900 to bg-zinc-100 dark:bg-zinc-900 
         - Changed border-white/5 to border-zinc-200 dark:border-white/5
      */}
      <div className="aspect-video w-full overflow-hidden relative bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-white/5 shadow-lg group-hover:shadow-xl transition-all duration-500 group-hover:-translate-y-1">
        <div className="absolute inset-0 p-3">
          <img
            ...
            alt={project.name}
            // Also updated the inner image border to match
            className="w-full h-full object-cover rounded-lg border border-zinc-200 dark:border-white/5"
          />
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 px-1">
        ...
        {/* Description */}
        <p className="text-base font-sans text-muted line-clamp-2">
          {project.description}
        </p>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            ...
            <p className="text-muted text-lg max-w-xl font-sans">
              Merging technical depth with creative design.
            </p>
          </div>

          <Link
            to="/all-projects"
            className="group flex items-center gap-2 text-lg font-bold font-syne text-page-dark dark:text-page hover:text-primary dark:hover:text-secondary transition-colors pb-1"
          >
            View All Projects
